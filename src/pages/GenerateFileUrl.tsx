...
import { Upload, FileCode, Loader2, Copy, Check, ExternalLink, FileUp } from 'lucide-react';
...
export default function GenerateFileUrl() {
  ...
  const handleGenerateLink = () => {
    ...
    setTimeout(() => {
      ...
      setPreviewLink(`https://files.example.com/share/${Math.random().toString(36).substring(7)}`);
    }, 1500);
  };
      ...
      <div className="flex justify-between items-end pb-4 border-b border-zinc-200/60 mb-6">
        <div className="flex flex-col space-y-1">
          <p className="text-[11px] text-zinc-700 font-normal tracking-wide">
            {isUploaded ? `Generated link for: ${fileName}` : 'Upload any file to generate a shareable download link.'}
          </p>
        </div>
          ...
          <div className="w-full max-w-2xl bg-white rounded-2xl border border-zinc-200/60 shadow-sm p-8">
            ...
            >
              <div className="w-16 h-16 bg-white border border-zinc-200/80 rounded-2xl shadow-sm flex items-center justify-center mb-6 group-hover:scale-105 transition-transform">
                <FileUp className="w-8 h-8 text-zinc-600 group-hover:text-zinc-900 transition-colors" strokeWidth={1.5} />
              </div>
              <h3 className="text-base font-medium text-zinc-900 tracking-tight mb-2">Upload Any File</h3>
        ...
        >
          {isGenerating ? (
            <div className="py-12 flex flex-col items-center justify-center border border-zinc-200/60 border-dashed rounded-2xl bg-white shadow-sm">
              ...
              <p className="text-[13px] text-zinc-700 font-normal">Generating file link...</p>
            </div>
          ) : previewLink ? (
