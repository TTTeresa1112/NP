...
import { Upload, Link as LinkIcon, FileCode, Loader2, Copy, Check } from 'lucide-react';
      ...
      <div className="flex justify-between items-end pb-4 border-b border-zinc-200/60 mb-6">
        <div className="flex flex-col space-y-1">
          <p className="text-[11px] text-zinc-700 font-normal tracking-wide">
            {isUploaded ? `Generating preview for: ${fileName}` : 'Upload an XML file to generate a shareable preview link.'}
          </p>
        </div>
          ...
          {isUploaded && (
            <>
              ...
              <button 
                ...
                disabled={isGenerating || !!previewLink}
              ...
              >
                ...
                {isGenerating ? 'Generating...' : previewLink ? 'Link Generated' : 'Generate Link'}
              </button>
            </>
          )}
        </div>
        ...
        >
          {previewLink ? (
            ...
            <div className="py-12 flex flex-col items-center justify-center border border-zinc-200/60 border-dashed rounded-2xl bg-white shadow-sm">
              <FileCode className="w-8 h-8 text-zinc-400 mb-3" strokeWidth={1.5} />
              <p className="text-[13px] text-zinc-700 font-normal">File ready. Click "Generate Link" to create a preview.</p>
            </div>
          )}
        </motion.div>
      )}
